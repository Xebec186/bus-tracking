...
export default function FareEstimateScreen({ route: navRoute, navigation }) {
  ...
  async function calculate() {
    ...
    if (!count || count < 1) {
      setError("Enter at least one passenger.");
      return;
    }
    setError(null);
    setCalcLoading(true);
    setEstimate(null);
    try {
      const res = await passengerApi.getFareEstimate({
        scheduleId: parseInt(
          (schedule?.id ?? schedule?.scheduleId).toString().split("_")[0],
        ),
        originStopId,
        destinationStopId,
      });
      const fare = unwrapApiData(res);
      setEstimate({
        fare,
        totalFare: Number(fare) * count,
        perPassenger: Number(fare),
      });
    } catch (err) {
      setError(
  ...
  if (loading)
    return <LoadingSpinner fullScreen message="Loading fare info…" />;
        ...
        <View style={styles.routeCard}>
          ...
          <View style={{ marginTop: SPACING.md }}>
            <Text style={styles.scheduleText}>
              ...
              {" · "}
              {schedule?.scheduleDay ?? schedule?.day ?? "Daily"}
            </Text>
          </View>
        </View>
        ...
        {estimate && (
          <View style={styles.resultCard}>
            ...
            <Text style={styles.resultAmount}>
              GH₵{" "}
              {estimate.totalFare ?? estimate.fare ?? estimate.amount ?? "—"}
            </Text>
            {estimate.currency && (
